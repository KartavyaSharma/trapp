...
class Handshake(Platform):
    ...
    def scrape(self) -> tuple[str, str, str, str]:
        inner_html = self.curr_driver.execute_script('return document.getElementById("skip-to-content").innerHTML')
        ...
        title_tags = soup.find_all(class_=get_class_from_tag(title_pattern.findall(inner_html)[0]))
        ...
        company_tags = soup.find_all(class_=get_class_from_tag(company_pattern.findall(inner_html)[0]))
        company = company_tags[0].get_text()
        # Define location pattern
        location_pattern = re.compile(r'class="style__list-with-tooltip___[a-zA-Z0-9]{5}"')
        location_tags = soup.find_all(class_=get_class_from_tag(location_pattern.findall(inner_html)[0]))
