...
class LoggerBuilder:
    ...
    def build(logfile: str = None, log_level: any = None) -> logging.Logger:
        ...
        """
        filename = (
            logfile.split("/")[-1] if logfile else constants.LOG_FILENAME.split("/")[-1]
        )
        ...
        new_logger_file_handler.setFormatter(
            logging.Formatter(constants.LOG_FORMAT, constants.LOG_DATEFMT)
        )
