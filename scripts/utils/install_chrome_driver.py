...
sys.path.append(f"{pathlib.Path(__file__).parent.resolve()}/../../") # Make constants accessible to utils
...
def main():
    ...
    subprocess.call(f"wget -O {constants.PROJECT_ROOT}/bin/chrome-driver/chrome-driver.zip {url}", shell=True)
    # Unzip chrome-driver
    print("Unzipping chrome-driver...")
    subprocess.call(f"unzip {constants.PROJECT_ROOT}/bin/chrome-driver/chrome-driver.zip -d {constants.PROJECT_ROOT}/bin/chrome-driver/", shell=True)
    # Remove chrome-driver.zip
    print("Removing chrome-driver.zip...")
    subprocess.call(f"rm {constants.PROJECT_ROOT}/bin/chrome-driver/chrome-driver.zip", shell=True)
    # Make chrome-driver executable
    print("Making chrome-driver executable...")
    subprocess.call(f"chmod +x {constants.PROJECT_ROOT}/bin/chrome-driver/chromedriver-{system_arch_identifier}", shell=True)
