...
def set_chrome_driver_cache(
    versions_json: any = None, version_root: str = None
) -> None:
    if not versions_json:
        ...
        possible_versions = [
            v["version"]
            for v in versions_json["versions"]
            if v["version"].startswith(version_root)
        ]
        highest_version_suffix = sorted(
            [int(v.split(".")[-1]) for v in possible_versions]
        )[-1]
        ...
        target_version_object = [
            v for v in versions_json["versions"] if v["version"] == resolved_version
        ...
        constructed_cache = {
            "timestamp": versions_json["timestamp"],
            "url": target_version_url,
        }
...
def get_chrome_driver_url(version: str) -> str:
    ...
    if r.status_code != 200:
        logger.LoggerBuilder.build(log_level=logging.ERROR).error(
            f"Failed to get Chrome driver versions: returned status code: {r.status_code}"
        )
